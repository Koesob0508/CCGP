...
namespace CCGP.Client
{
    ...
    public class ClientNotificationCenter
    {
        ...
        public void AddObserver(Handler handler, string notificationName, object sender = null)
        {
            if (handler == null)
            {
                LogUtility.LogWarning<ClientNotificationCenter>($"Can't add a null event handler for notification, {notificationName}");
                return;
            }

            if (string.IsNullOrEmpty(notificationName))
            {
                LogUtility.LogWarning<ClientNotificationCenter>("Can't observe an unnamed notification");
                return;
            }
        ...
        public void RemoveObserver(Handler handler, string notificationName, object sender = null)
        {
            if (handler == null)
            {
                LogUtility.LogWarning<ClientNotificationCenter>("Can't remove a null event handler for notification, " + notificationName);
                return;
            }

            if (string.IsNullOrEmpty(notificationName))
            {
                LogUtility.LogWarning<ClientNotificationCenter>("A notification name is required to stop observation");
                return;
            }
        ...
        public void PostNotification(string notificationName, object sender = null, object e = null)
        {
            if (string.IsNullOrEmpty(notificationName))
            {
                LogUtility.LogWarning<ClientNotificationCenter>("A notification name is required");
                return;
            }
    ...
    public static class ClientNotificationExtensions
    {
        ...
        {
            ClientNotificationCenter.Instance.PostNotification(notificationName, obj, e);
        }

        public static void AddObserver(this object obj, Handler handler, string notificationName, object sender = null)
        {
            ClientNotificationCenter.Instance.AddObserver(handler, notificationName, sender);
        }

        public static void RemoveObserver(this object obj, Handler handler, string notificationName, object sender = null)
        {
            ClientNotificationCenter.Instance.RemoveObserver(handler, notificationName, sender);
        }
    }
}