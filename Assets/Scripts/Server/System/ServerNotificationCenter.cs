...
namespace CCGP.Server
{
    ...
    public class ServerNotificationCenter
    {
        #region Properties
        /// <summary>
        ...
        public static ServerNotificationCenter Instance { get; private set; } = new ServerNotificationCenter();
        private ServerNotificationCenter() { }
        public static void ResetInstacne()
        {
            Instance = new ServerNotificationCenter();
        }
        #endregion
        ...
        public void AddObserver(Handler handler, string notificationName, object sender = null)
        {
            if (handler == null)
            {
                LogUtility.LogWarning<ServerNotificationCenter>($"Can't add a null event handler for notification, {notificationName}");
                return;
            }

            if (string.IsNullOrEmpty(notificationName))
            {
                LogUtility.LogWarning<ServerNotificationCenter>("Can't observe an unnamed notification");
                return;
            }
        ...
        public void RemoveObserver(Handler handler, string notificationName, object sender = null)
        {
            if (handler == null)
            {
                LogUtility.LogWarning<ServerNotificationCenter>("Can't remove a null event handler for notification, " + notificationName);
                return;
            }

            if (string.IsNullOrEmpty(notificationName))
            {
                LogUtility.LogWarning<ServerNotificationCenter>("A notification name is required to stop observation");
                return;
            }
        ...
        public void PostNotification(string notificationName, object sender = null, object e = null)
        {
            if (string.IsNullOrEmpty(notificationName))
            {
                LogUtility.LogWarning<ServerNotificationCenter>("A notification name is required");
                return;
            }
    ...
    public static class ServerNotificationExtensions
    {
        ...
        {
            ServerNotificationCenter.Instance.PostNotification(notificationName, obj, e);
        }

        public static void AddObserver(this object obj, Handler handler, string notificationName, object sender = null)
        {
            ServerNotificationCenter.Instance.AddObserver(handler, notificationName, sender);
        }

        public static void RemoveObserver(this object obj, Handler handler, string notificationName, object sender = null)
        {
            ServerNotificationCenter.Instance.RemoveObserver(handler, notificationName, sender);
        }
    }
}