    ...
    public class TargetSystem : Aspect, IObserve
    {
        ...
        private IEnumerator WaitTargetSelect(IContainer game, GameAction action)
        {
            ...
            if (cardPlayAction.Card.Space.Contains(target.Selected.Space))
            {
                ...
                Logger.Log<TargetSystem>($"선택된 타겟: {target.Selected.Name}");
            }
            else
            {
                // 만일 잘못된 경우, action.Cancel()을 낸다.
                Logger.LogWarning<TargetSystem>("잘못된 타겟이 선택됨. 카드 발동이 취소됩니다.");
                action.Cancel();
            }
        ...
        public void SetTarget(CardPlayAction action, Tile selectedTile)
        {
            ...
            {
                Logger.LogWarning<TargetSystem>("Action이나 selectedTile이 null입니다.");
                return;
            }
            ...
            if (action.Card.TryGetAspect(out Target target))
            {
                ...
                Logger.Log<TargetSystem>($"타겟이 '{selectedTile.Name}'으로 설정되었습니다.");
            }
            else
            {
                Logger.LogWarning<TargetSystem>("해당 카드에 Target Aspect가 없습니다.");
            }
        }
