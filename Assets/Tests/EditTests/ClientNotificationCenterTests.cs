...
{
    public class ClientNotificationCenterTests
    {
        [SetUp]
        public void Setup()
        {
            // 매 테스트마다 싱글톤 인스턴스를 리셋합니다.
            ClientNotificationCenter.ResetInstance();
        }
        ...
        public void AddObserverAndPostNotification_CallsHandler()
        {
            ...
            // Act
            ClientNotificationCenter.Instance.AddObserver(handler, notificationName);
            ClientNotificationCenter.Instance.PostNotification(notificationName, testSender, testData);
        ...
        public void RemoveObserver_HandlerNotCalledAfterRemoval()
        {
            ...
            // Act
            ClientNotificationCenter.Instance.AddObserver(handler, notificationName);
            ClientNotificationCenter.Instance.RemoveObserver(handler, notificationName);
            ClientNotificationCenter.Instance.PostNotification(notificationName, null, null);
        ...
        public void MultipleObservers_AllCalledAndRemovalWorksCorrectly()
        {
            ...
            ClientNotificationCenter.Instance.AddObserver(handler1, notificationName);
            ClientNotificationCenter.Instance.AddObserver(handler2, notificationName);
            ClientNotificationCenter.Instance.PostNotification(notificationName, null, null);
            ...
            ClientNotificationCenter.Instance.RemoveObserver(handler1, notificationName);
            ClientNotificationCenter.Instance.PostNotification(notificationName, null, null);
        ...
        public void ObserverWithSpecificSender_OnlyCalledWhenSenderMatches()
        {
            ...
            ClientNotificationCenter.Instance.AddObserver(handler, notificationName, specificSender);

            // 다른 sender로 PostNotification 하면 호출되지 않아야 함.
            ClientNotificationCenter.Instance.PostNotification(notificationName, new object(), null);
            Assert.IsFalse(wasCalled, "Handler should not be called if sender does not match.");

            // 올바른 sender로 알림을 Post하면 호출되어야 함.
            ClientNotificationCenter.Instance.PostNotification(notificationName, specificSender, null);
        ...
        public void RemoveObserverDuringNotification_HandlerNotCalledInLaterNotifications()
        {
            ...
            ClientNotificationCenter.Instance.PostNotification(notificationName, null, "FirstCall");
            // 두 번째 알림에서는 제거된 Observer가 호출되지 않아야 함
            ClientNotificationCenter.Instance.PostNotification(notificationName, null, "SecondCall");
